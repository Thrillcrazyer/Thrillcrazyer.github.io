...
현대 RNN 연구에서 GRU <d-cite key="Chung2014EmpiricalEO"></d-cite> 이후의 시대정신은 **시퀀스의 정보를 어떤 구조로 분해해 기억할 것인가**에 있었다고 생각합니다.
RNN은 과거의 정보를 고정된 차원의 hidden state에 압축해 담기 때문에 결국 기억의 용량이 제한된 상황에서 핵심은 정보를 얼마나 많이 담느냐보다 어떤 기저와 구조로 분해해 어떤 정보를 잘 보존해서 남기느냐에 있었습니다.

대표적으로 시계열의 추세와 계절성간의 상관 구조를 STL을 통해 분해한 CRU<d-cite key="sim2023cru"></d-cite> 와 Fourier docomposition 을 기반으로 주파수를 기저로 분해한 FRU<d-cite key="zhang2018fru"></d-cite>, 그리고 Legendre 다항식을 기반으로 시계열을 분해한 LMU<d-cite key="zhang2018fru"></d-cite> 등이 있습니다.
...
앞서 본 방법들의 공통점은 과거 입력을 그대로 저장하는 대신, 특정한 inductive bias를 담은 basis 위의 vector로 변환해 기억한다는 데 있습니다.
무엇을 쉽게 보존하고 무엇을 잊을지는 basis와 measure의 선택이 정합니다. HiPPO는 이 선택을 inner product가 정의된 function space 위의 optimal projection 문제로 정식화하고, 과거 signal을 계속 입력 받으면서 이들에 대한 가장 optimal 한 finite-dimensional coefficient vector로 유지합니다(**Online Approximation**). 이 vector들은 같은 basis와 measure를 공유하므로, 서로 다른 시점의 memory를 같은 좌표계에서 비교하고 inner product를 비롯한 선형 연산을 일관되게 적용할 수 있습니다.
...
HiPPO는 지금까지 관측한 함수 이력 $f_{\leq t}$의 orthogonal polynomial space로의 projection으로 정의합니다. 시점 $t$의 approximation은 다음과 같이 표현할 수 있습니다.<d-cite key="gu2020hippo"></d-cite>
...
여기서 $\mu_t$는 basis에 대한 measure이고, $\phi_i^{(t)}$는 orthogonal basis function입니다. HiPPO는 전체 이력 대신 coefficient vector $c(t)$만 유지하면서, 선택한 measure와 basis에 맞는 update rule을 유도합니다.

$$
\frac{d}{dt}c(t)=-A(t)c(t)+B(t)f(t).
$$

대표적인 LegS와 LagT는 과거를 보는 measure가 다릅니다.
...
이 measure와 basis로부터 유도되는 update rule은 위의 $\dot c(t)=-A(t)c(t)+B(t)f(t)$ 형태의 linear state-space system이 됩니다.<d-cite key="gu2020hippo"></d-cite>

그리고 이러한 LTI 시스템을 이산화 하게 되면 다음과 같은 형태로 변환할 수 있습니다.
...
입니다. 대각 원소는 $n+1$이고, lower-triangular 부분은 두 basis index에 따라 정해지는 square-root 계수입니다.

이에 대응해 LagT는 훨씬 단순한 unit lower-triangular matrix로 쓸 수 있습니다.

$$
\widetilde A^{\mathrm{LagT}}=
\begin{bmatrix}
1      &        &        & \cdots\\
1      & 1      &        & \cdots\\
1      & 1      & 1      & \cdots\\
...
두 matrix 모두 임의로 학습한 dense matrix가 아니라, 선택한 basis와 measure로부터 정해지는 규칙적인 구조입니다. LegS는 지금까지의 전체 이력을 같은 상대적 시간 축에서 근사하므로, basis의 차수에 따라 square-root 계수가 달라집니다. 반면 LagT는 오래된 정보를 지수적으로 덜 반영하므로, 모든 과거 coefficient를 같은 방식으로 누적하는 unit lower-triangular 구조를 갖습니다.
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 4. Hippo Framework.</figcaption>
</figure>

제가 보기에 HiPPO의 가장 큰 전환점은 특정 다항식을 사용했다는 사실 자체보다, 과거 전체를 **coefficient vector** $c(t)$ 하나로 다룰 수 있게 했다는 점입니다. 즉, 모델은 이전 입력을 모두 저장하는 대신 "**각 basis가 과거 signal을 얼마나 설명하는가**"만 저장합니다.
이 표현에서는 서로 다른 시점의 memory가 같은 basis 위의 vector이므로, 두 memory를 비교하거나 합하고 선형 변환하는 연산을 일관되게 정의할 수 있습니다.
그리고 이 압축된 memory가 새 입력을 만날 때 어떻게 바뀔지는 $A(t)$와 $B(t)$가 결정합니다. 다시 말해 basis와 measure로 정한 기억 방식이 상태 전이 행렬의 구조로 나타나는 것입니다.

## After HiPPO

HiPPO가 남긴 다음 과제는 function space에서 measure로 정의한 memory를, 실제 recurrent model의 유한한 $A,B$ update로 어떻게 구현할 것인가입니다. 새 observation이 들어와도 과거 전체를 다시 보지 않고 coefficient를 갱신하려면, 원하는 projection의 update가 $A,B$만으로 닫혀야 합니다. 그리고 긴 sequence에서 이 update를 효율적으로 계산하려면 $A$의 구조를 활용할 수 있어야 합니다. **Low recurrence width(LRW)**는 바로 이 계산적 성질을 설명하는 개념입니다.<d-cite key="desa2018lrw"></d-cite>

<aside class="margin-note">
<strong>Low recurrence width</strong>는 matrix의 행을 짧은 polynomial recurrence로 생성할 수 있는지를 나타냅니다. $i$번째 행 $A[i,:]$를 계수로 하는 polynomial $a_i(X)$를 만들고, 바로 앞의 $r$개 행만으로 $a_i(X)$를 표현할 수 있으면 recurrence width가 $r$입니다. 이때 $g_{i,j}(X)$의 차수를 $j$ 이하로 제한하면, 멀리 떨어진 행이나 임의의 긴 coefficient 목록을 참조하지 않고도 다음 행을 생성할 수 있습니다. 작은 $r$은 matrix 전체를 dense하게 저장하고 계산하는 대신, 짧은 recurrence를 이용하는 빠른 matrix-vector multiplication의 출발점이 됩니다. LRW는 projection의 정확성 자체를 보장하는 조건이라기보다, 이미 유도된 structured update를 효율적으로 구현할 수 있게 하는 계산적 성질입니다.
</aside>

행렬 $A\in\mathbb{R}^{N\times N}$의 $i$번째 행을 다항식 $a_i(X)=\sum_{k=0}^{N-1}A[i,k]X^k$로 나타낼 때, recurrence width $r$은 다음 관계로 정의됩니다.
...
<aside class="margin-note">
<strong>LRW example</strong>: 이항계수 lower-triangular matrix의 $i$번째 행을 polynomial로 쓰면 $a_i(X)=(1+X)^i$입니다. 따라서 $a_i(X)=(1+X)a_{i-1}(X)$로 바로 이전 행 하나만으로 다음 행을 생성할 수 있으므로 $r=1$입니다. 이 행렬이 HiPPO의 $A$ 자체는 아니지만, structured $A$가 긴 sequence의 계산량을 제어하는 방식을 보여 줍니다.
</aside>
LRW는 projection의 정확성이나 학습 성능을 직접 보장하는 조건은 아닙니다. 대신 HiPPO에서 유도한 structured update를 빠르게 계산할 수 있게 해 줍니다. 이후의 현대 SSM 연구는 이 출발점 위에서, 상태 전이 $A$에 얼마나 많은 표현력을 허용할지와 그 계산을 어떻게 효율적으로 유지할지를 함께 탐구합니다.
...
아래 표는 이 관점에서 본 현대 딥러닝 SSM의 발전을 요약합니다. 관련 배경은 [발표자료](https://pnubaelab.github.io/blog/2025/Mamba/)에도 정리되어 있습니다.

| 연구   | 핵심 아이디어                                                                                                                                                              |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LSSL<d-cite key="gu2021lssl"></d-cite>   | 그냥 $A$ Matrix 학습할 때 초기화를 Legendre polynomial로 하자 .                                                     |
| S4<d-cite key="gu2022s4"></d-cite>     | LRW 구조는 diagonal plus low-rank(DPLR) 형태로 표현할 수 있으니, DPLR로 제약을 주고 학습하자!.                            |
| S4D<d-cite key="gu2022s4d"></d-cite>    | DPLR 에서 Diagonal 구조만 사용하자!.                                                                 |
| Mamba<d-cite key="gu2024mamba"></d-cite>  | A Matrix는 Time-Dependent하게 만들어서 x로 A matrix를 조절하자!.                                             |
| Mamba2<d-cite key="dao2024mamba2"></d-cite> | A Matrix를 Diagonal 구조에서 더 Ideal 하게 $\alpha I$로 보고 scalar 값으로 만들어서 Linear Transformer 구조와 결합하자. |
...
Mamba2까지는 Next State를 업데이트 하는 문제를 **Online Approximation** 관점에서 점을 했다면, Longhorn<d-cite key="liu2025longhorn"></d-cite>은 이를 좀 더 일반화하여 **Online Convex Programming**<d-cite key="zinkevich2003online"></d-cite>로 해석합니다. 즉, 현재 observation에 맞추되 이전 state에서 과도하게 벗어나지 않도록 update를 결정하는 방식입니다.
이 논문에서 가장 흥미롭게 본 부분은 새로운 recurrence를 하나 더 제안했다는 점보다, recurrence를 설계하는 질문 자체를 바꾼 점입니다.
HiPPO 이후의 흐름이 “어떤 $A$를 써서 잘 압축할 수 있을 것인가”를 물었다면, Longhorn은 **“어떤 목적함수를 풀어서 가장 최적의 압축을 상태를 찾을 수 있는가”**를 묻습니다. 기본적인 형태는 다음과 같습니다.
...
즉, 두 항은 각각 과거를 지키려는 힘과 현재에 적응하려는 힘으로 각각의 힘을 찾는 것이라고 볼 수 있습니다.
...
<section class="longhorn-regression-explorer" data-longhorn-regression-explorer aria-label="Online linear regression explorer">
  <header class="longhorn-regression-explorer__intro">
    ...
    <p>각 observation $(x_t,y_t)$가 도착하면, 이전 직선에서 너무 멀어지지 않으면서 현재 점의 prediction error를 줄이는 $\theta_t=[b_t,w_t]$를 선택합니다.</p>
  </header>
  ...
  <div class="longhorn-regression-explorer__body">
    <div class="longhorn-regression-explorer__stage" aria-live="polite">
      ...
      <div class="longhorn-regression-explorer__state">
        ...
        <p class="longhorn-regression-explorer__residual">pre-update residual <strong data-longhorn-residual>0.00</strong></p>
      </div>
    </div>
...
슬라이드의 채널별 Longhorn 목적함수는 다음과 같습니다.
...
설명을 위해 모든 채널이 같은 스칼라 $\beta_t\geq0$를 사용한다고 두면, 목적함수와 그 정확한 해는 다음과 같이 단순해집니다.
...
현재 메모리의 예측 오차를 이용해 상태를 수정하며, 분모가 유효 업데이트 크기를 조절합니다.
실제 Longhorn은 채널별 $\boldsymbol{\beta}_t\in(0,1)^{d_v}$를 사용합니다.
각 채널의 정확한 closed-form update는 다음과 같습니다.<d-cite key="liu2025longhorn"></d-cite>
...
$$
S_{t}
=S_{t-1}(I-\varepsilon_{t,i}k_tk_t^\top)
+\varepsilon_{t,i}v_{t,i}k_t^\top.
$$
...
Longhorn 이후 제가 중요하게 본 변화는, sequence mixer를 새로운 learning objective와 그에 대응하는 update rule의 쌍으로 설명하기 시작했다는 점입니다. 발표에서 사용한 표기처럼, 현재 state를 크게 만들고 싶은 score $L_t$에 대해 다음 objective를 둘 수 있습니다.
...
Linear Attention<d-cite key="katharopoulos2020linear"></d-cite> 계열에서 state matrix $S$는 key를 value로 연결하는 associative memory로 볼 수 있습니다. 현재 key $k_t$를 읽으면 $Sk_t$가 model이 예측한 value가 되고, LA의 현재 정보 항은 $\langle Sk_t,v_t\rangle$를 크게 만들어 두 vector의 alignment를 높입니다. 이 score의 $S$에 대한 gradient는
...
대표적인 online learning objective와 그에 대응하는 update를 현재 글의 notation으로 정리하면 다음과 같습니다.<d-cite key="liu2025longhorn"></d-cite>
...
DeltaNet<d-cite key="yang2024deltanet"></d-cite>과 Gated DeltaNet<d-cite key="yang2025gateddeltanet"></d-cite>은 앞에서 본 online learning 관점을 residual correction으로 확장합니다. 핵심은 새 key-value pair를 그대로 더하는 대신, 현재 memory가 key $k_t$에 대해 만든 prediction error를 write target으로 사용한다는 점입니다.
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 5. GDN Framework.</figcaption>
</figure>
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Table. GDN 성능.</figcaption>
</figure>
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 7. Kimi Delta Attention.</figcaption>
</figure>
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 6. Kimi Linear Framework.</figcaption>
</figure>
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 8. Kimi Delta Attention.</figcaption>
</figure>

Kimi Linear는 KDA를 Multi-Head Latent Attention(MLA)과 결합한 hybrid architecture로 평가합니다. 실험에서는 MLA-only 구성과 비교해 성능을 높이면서도, recurrent KDA 계층을 활용해 decoding throughput과 TPOT(time per output token)을 효율적으로 유지하는 결과를 제시합니다. 다만 이 결과는 모델 크기, 학습량, context length, hardware 조건을 함께 고려해 해석해야 합니다.
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Table. Kimi Delta Attention.</figcaption>
</figure>

논문은 KDA와 MLA의 mixture를 ablation으로 비교했고, 제시한 설정에서는 **KDA:MLA = 3:1**을 사용합니다.<d-cite key="kimi2025linear"></d-cite> 이 비율을 모든 hybrid model의 표준으로 일반화하기보다, memory budget과 task에 따라 달라지는 설계 선택으로 보는 편이 적절합니다.

## Online Learning 모델 근황

최근 Online Learning base 기반으로 Attention 모델들을 기존의 GQA 나 Dense Attention 과 결합하는 사례가 잦아지고 있음. 성능 좋아지고 빨라짐 ㅇㅇ
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 9. Qwen 3.8 Next Architecture.</figcaption>
</figure>
...
<figure>
  <img
      ...
      loading="lazy"
    >
    <figcaption>Figure 10. Solar Pro Architecture.</figcaption>
</figure>
